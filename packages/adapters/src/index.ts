...
export function adaptersFromEnvironment(env: NodeJS.ProcessEnv = process.env): ModelAdapter[] {
  ...
  if (env.MISTRAL_API_KEY) adapters.push(new OpenAICompatibleAdapter({ id: "mistral:balanced", provider: "mistral", model: env.MISTRAL_MODEL ?? "mistral-large-latest", apiKey: env.MISTRAL_API_KEY, baseUrl: env.MISTRAL_BASE_URL ?? "https://api.mistral.ai/v1", inputCostPerMillion: 0.5,outputCostPerMillion:1.5 }));
  if (env.QWEN_API_KEY) adapters.push(new OpenAICompatibleAdapter({ id: "qwen:balanced", provider: "qwen", model: env.QWEN_MODEL ?? "qwen3.7-max", apiKey: env.QWEN_API_KEY, baseUrl: env.QWEN_BASE_URL ?? "https://dashscope-intl.aliyuncs.com/compatible-mode/v1", inputCostPerMillion: 2.5,outputCostPerMillion:7.5 }));
