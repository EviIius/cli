...
test("configures OpenAI-compatible Qwen, Mistral, Gemini, and Gemma routes",()=>{
  ...
  for(const id of ["gemini:fast","gemini:balanced"]){
    const adapter=adapters.find(item=>item.id===id);
    assert.ok(adapter,`${id} should be configured`);
    assert.equal(adapter.provider,"gemini");
    assert.equal(adapter.capabilities.selfHosted,false);
  }
});
