...
export function createApp(env: NodeJS.ProcessEnv = process.env) {
  ...
  app.post("/v1/jobs",async(request,reply)=>{const actor=guard(request,reply,["owner","admin","builder"]);if(!actor)return;const body=request.body as {type?:"single"|"planner-reviewer"|"adhoc";objective?:string;priority?:"fast"|"balanced"|"deep"|"private";steps?:import("@relay/workers").WorkflowStep[]};if(!body.objective||!body.type||!["single","planner-reviewer","adhoc"].includes(body.type))return reply.code(400).send({error:"a valid type and objective are required"});try{return reply.code(202).send(await workflows.submit(actor.tenantId,actor.userId,body.type,{objective:body.objective,priority:body.priority,steps:body.steps}));}catch(error){return reply.code(400).send({error:error instanceof Error?error.message:"Job submission failed"});}});
