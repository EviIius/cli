...
type TokenPayload = AuthPrincipal & { exp: number; iat: number };
...
class AuthRepository {
  ...
  async ensureDevelopmentPrincipal(): Promise<void> {
    ...
    try {
      ...
      await client.query(`INSERT INTO tenants(id,slug,name,policy) VALUES($1::uuid,'relay-development','Relay Development',$2::jsonb) ON CONFLICT(id) DO NOTHING`,[defaultTenantId,JSON.stringify({tenantId:defaultTenantId,allowedProviders:["openai","anthropic","mistral","qwen","gemma","local","demo"],externalProvidersAllowed:true,monthlyBudgetUsd:100,writeToolsRequireApproval:true})]);
...
export class AuthService {
  ...
  constructor(env: NodeJS.ProcessEnv = process.env) { this.repository = new AuthRepository(env.DATABASE_URL); this.secret = env.AUTH_SECRET ?? "relay-development-secret-change-before-production"; this.mode = env.AUTH_MODE === "required" ? "required" : "optional"; if (this.mode === "required" && !env.AUTH_SECRET) throw new Error("AUTH_SECRET is required when AUTH_MODE=required"); }
  ...
  private sign(user: AuthPrincipal): string { const now=Math.floor(Date.now()/1000); const body=encode({...this.principal(user),iat:now,exp:now+8*60*60}); return `${body}.${createHmac("sha256",this.secret).update(body).digest("base64url")}`; }
