...
class AuthRepository {
  ...
  async ensureDevelopmentPrincipal(): Promise<void> {
    ...
    try {
      ...
      await client.query(`INSERT INTO tenants(id,slug,name,policy) VALUES($1::uuid,'relay-development','Relay Development',$2::jsonb) ON CONFLICT(id) DO NOTHING`,[defaultTenantId,JSON.stringify({tenantId:defaultTenantId,allowedProviders:["openai","anthropic","mistral","qwen","local","demo"],externalProvidersAllowed:true,monthlyBudgetUsd:100,writeToolsRequireApproval:true})]);
